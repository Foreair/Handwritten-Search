...
public class PenSample1_2_PenSetting extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpenSurfaceView.setPenChangeListener(onPenChangeListenner);
        mSpenSurfaceView.setOnTouchListener(onTouchSurfaceViewListener);
    ...
    private final OnTouchListener onTouchSurfaceViewListener = new OnTouchListener() {
        ...
        public boolean onTouch(View view, MotionEvent event) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                if (mSpenSurfaceView.getToolTypeAction(SpenSurfaceView.TOOL_FINGER) == SpenSurfaceView.ACTION_STROKE
                        || mSpenSurfaceView.getToolTypeAction(SpenSurfaceView.TOOL_SPEN) == SpenSurfaceView.ACTION_STROKE) {
                    mPenSettingView.setVisibility(View.GONE);
             ...
             //   List<SpenPenPresetInfo> presetList = mPenSettingView.getPenPresetInfoList();
                for (int i = 0; i < mFavoriteList.size(); i++) {
                    ...
                    if (!mFavoriteList.get(i).advancedSetting.equalsIgnoreCase(info.advancedSetting)
                            && "com.samsung.android.sdk.pen.pen.preload.Beautify".equals(mFavoriteList.get(i).name)) {
                        continue;
                    }
